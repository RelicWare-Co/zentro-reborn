...
export const creditAccount = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("credit_org_customer_uidx").on(table.organizationId, table.customerId),
  ]
);
...
export const creditTransaction = sqliteTable(
  "credit_transaction",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id").notNull().references(() => organization.id, { onDelete: "cascade" }),
    creditAccountId: text("credit_account_id").notNull().references(() => creditAccount.id, { onDelete: "cascade" }),
