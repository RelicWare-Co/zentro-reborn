...
import { Building2, ChevronLeft, ChevronRight, LayoutDashboard, Loader2, LogOut, Menu, Package, Settings, Store } from 'lucide-react'
    ...
    <div className="flex h-screen bg-[var(--color-void)] text-[var(--color-photon)] overflow-hidden">
      ...
      `}>
        ...
        <div className="h-16 flex items-center px-5 border-b border-gray-800 whitespace-nowrap overflow-hidden shrink-0">
          <Link to="/" className={`block overflow-hidden text-2xl font-bold tracking-tight text-[var(--color-voltage)] transition-all duration-300 ${isCollapsed ? 'w-[120px] opacity-100 lg:w-0 lg:opacity-0 lg:pointer-events-none' : 'w-[120px] opacity-100'}`}>
            Zentro
          </Link>
          <button
            type="button"
        ...
        <div className={`p-4 border-b border-gray-800 shrink-0 ${isCollapsed ? 'lg:px-2' : ''}`}>
          <button 
            type="button"
            onClick={async () => {
          ...
          >
            ...
            <div className={`transition-all duration-300 overflow-hidden ${isCollapsed ? 'w-auto opacity-100 ml-3 lg:w-0 lg:opacity-0 lg:ml-0 lg:flex-none' : 'w-auto opacity-100 ml-3 flex-1'}`}>
              <p className="text-sm font-medium text-white truncate">{activeOrganization?.name || "Sin organización"}</p>
        ...
        {/* Navigation */}
        <nav className={`flex-1 py-6 px-4 space-y-2 overflow-hidden ${isCollapsed ? 'lg:px-2' : ''}`}>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`transition-all duration-300 overflow-hidden ${isCollapsed ? 'w-[140px] opacity-100 ml-3 lg:w-0 lg:opacity-0 lg:ml-0' : 'w-[140px] opacity-100 ml-3'}`}>
                  {item.name}
                </span>
              </Link>
            )
        ...
        {/* Sidebar Footer */}
        <div className={`p-4 border-t border-gray-800 overflow-hidden ${isCollapsed ? 'lg:px-2' : ''}`}>
          <button 
            type="button"
            onClick={async () => {
          ...
          >
            ...
            <span className={`transition-all duration-300 overflow-hidden text-left ${isCollapsed ? 'w-[140px] opacity-100 ml-3 lg:w-0 lg:opacity-0 lg:ml-0' : 'w-[140px] opacity-100 ml-3'}`}>
              Cerrar sesión
            </span>
          </button>
        </div>
